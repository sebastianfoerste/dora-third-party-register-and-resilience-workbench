...
try {
  ...
  database.exec("CREATE TABLE IF NOT EXISTS _collaboration_migrations (name TEXT PRIMARY KEY, appliedAt TEXT NOT NULL)");
  const migrations = readdirSync("prisma/migrations", { withFileTypes: true })
    .filter((entry) => entry.isDirectory())
    .map((entry) => entry.name)
    .sort();
  const existing = new Set(database.prepare("SELECT name FROM _collaboration_migrations").all().map((row) => row.name));
  const userTables = database.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name NOT LIKE 'sqlite_%' AND name != '_collaboration_migrations'").all();
  ...
  for (const migration of migrations) {
    ...
    try {
      ...
      database.prepare("INSERT INTO _collaboration_migrations (name, appliedAt) VALUES (?, ?)").run(migration, new Date().toISOString());
